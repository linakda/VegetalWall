...
namespace Mur_Vegetal.Pages
{
    public class NewsAdminModel : PageModel{
        ...
        public void OnGet(){
            ...
            var requestNew = Query.Get("http://iotdata.yhdf.fr/api/web/events");
            if(requestNew == "Error" || String.IsNullOrEmpty(requestNew)){
                _ResultViewAdminNews = "<div class=\"news-param\">Error Api</div>";
            }
            else{
                var result = JsonConvert.DeserializeObject<List<News>>(requestNew);
